    ...
    if st.button("Login"):
        if username not in user_db:
            ...
            if encrypted == user_db[username]['encrypted']:
                ...
                # Store login details
                user_db[username]['last_token'] = token
                login_history[username]['count'] += 1
                login_history[username]['timestamps'].append(login_time)
                login_history[username]['tokens'].append(token)
...
# Page 3: View Users
    ...
    if not user_db:
        ...
        for username, data in user_db.items():
            ...
            st.text(f"🕓 Total Logins: {login_history[username]['count']}")
            
            with st.expander("📜 Login History"):
                for i, timestamp in enumerate(login_history[username]['timestamps']):
                    token = login_history[username]['tokens'][i]
